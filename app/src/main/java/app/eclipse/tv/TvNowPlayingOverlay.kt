...
class TvNowPlayingOverlay(
    private val root: FrameLayout,
    private val onPrevious: () -> Unit = { PlaybackBridge.previous(root.context) },
    private val onNext: () -> Unit = { PlaybackBridge.next(root.context) }
) {
    ...
    private lateinit var fastForward: ImageButton
    ...
    init {
        ...
        val timeRow = LinearLayout(root.context).apply {
            gravity = Gravity.CENTER_VERTICAL
            setPadding(0, 14, 0, 0)
        }
        ...
        duration.text = "0:00"
        rewind = button(android.R.drawable.ic_media_rew, "Seek back 10 seconds")
        fastForward = button(android.R.drawable.ic_media_ff, "Seek forward 10 seconds")
        rewind.setPadding(6, 6, 6, 6)
        fastForward.setPadding(6, 6, 6, 6)
        progress.max = 1000
        progress.progress = 0
        progress.isFocusable = false
        timeRow.addView(elapsed, LinearLayout.LayoutParams(44, -2))
        timeRow.addView(rewind, LinearLayout.LayoutParams(48, 40))
        timeRow.addView(progress, LinearLayout.LayoutParams(0, 20, 1f))
        timeRow.addView(fastForward, LinearLayout.LayoutParams(48, 40))
        timeRow.addView(duration, LinearLayout.LayoutParams(44, -2))
        panel.addView(timeRow)

        previous.setOnClickListener { onPrevious(); flash() }
        play.setOnClickListener { PlaybackBridge.playPause(root.context); flash() }
        next.setOnClickListener { onNext(); flash() }
        rewind.setOnClickListener { PlaybackBridge.seekBy(root.context, -10_000L); flash() }
        fastForward.setOnClickListener { PlaybackBridge.seekBy(root.context, 10_000L); flash() }
    ...
    fun hide() {
        visible = false
        hideAnimator?.cancel()
        panel.animate().alpha(0f).setDuration(180).withEndAction { panel.visibility = View.GONE }.start()
    }
    ...
    fun handleDpad(keyCode: Int): Boolean {
        ...
        when (keyCode) {
            KeyEvent.KEYCODE_DPAD_LEFT -> { onPrevious(); flash(); return true }
            KeyEvent.KEYCODE_DPAD_RIGHT -> { onNext(); flash(); return true }
            KeyEvent.KEYCODE_DPAD_CENTER -> { PlaybackBridge.playPause(root.context); flash(); return true }
            KeyEvent.KEYCODE_DPAD_UP -> { PlaybackBridge.seekBy(root.context, 10_000L); flash(); return true }
            KeyEvent.KEYCODE_DPAD_DOWN -> { PlaybackBridge.seekBy(root.context, -10_000L); flash(); return true }
        }
        return false
    }

    private fun flash() {
        panel.animate().scaleX(0.995f).scaleY(0.995f).setDuration(60).withEndAction {
            panel.animate().scaleX(1f).scaleY(1f).setDuration(60).start()
        }.start()
    }
