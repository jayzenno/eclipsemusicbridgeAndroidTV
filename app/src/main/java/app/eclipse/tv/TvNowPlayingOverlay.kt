...
class TvNowPlayingOverlay(private val root: FrameLayout) {
    ...
    init {
        ...
        val text = LinearLayout(root.context).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_VERTICAL; setPadding(28, 0, 12, 0) }
        title.textSize = 27f; title.setTextColor(Color.WHITE); title.typeface = Typeface.DEFAULT_BOLD; title.maxLines = 1; title.ellipsize = android.text.TextUtils.TruncateAt.END
        artist.textSize = 18f; artist.setTextColor(0xffd0d0d8.toInt()); artist.maxLines = 1; artist.ellipsize = android.text.TextUtils.TruncateAt.END
        album.textSize = 14f; album.setTextColor(0xff9999a2.toInt()); album.maxLines = 1; album.ellipsize = android.text.TextUtils.TruncateAt.END
        status.textSize = 12f; status.setTextColor(0xffdddddd.toInt()); status.typeface = Typeface.DEFAULT_BOLD; status.setPadding(0, 9, 0, 0)
        text.addView(title); text.addView(artist); text.addView(album); text.addView(status)
        ...
        previous = button(android.R.drawable.ic_media_previous, "Previous"); play = button(android.R.drawable.ic_media_play, "Play"); next = button(android.R.drawable.ic_media_next, "Next")
        controls.addView(previous); controls.addView(play); controls.addView(next); row.addView(controls, LinearLayout.LayoutParams(190, -1)); panel.addView(row)
        val timeRow = LinearLayout(root.context).apply { gravity = Gravity.CENTER_VERTICAL; setPadding(0, 14, 0, 0) }
        elapsed.textSize = 12f; elapsed.setTextColor(0xffaaaaaf.toInt()); elapsed.text = "0:00"
        duration.textSize = 12f; duration.setTextColor(0xffaaaaaf.toInt()); duration.text = "0:00"
        progress.max = 1000; progress.progress = 0; progress.isFocusable = false
        timeRow.addView(elapsed, LinearLayout.LayoutParams(44, -2)); timeRow.addView(progress, LinearLayout.LayoutParams(0, 20, 1f)); timeRow.addView(duration, LinearLayout.LayoutParams(44, -2)); panel.addView(timeRow)
        previous.setOnClickListener { PlaybackBridge.previous(root.context); flash() }; play.setOnClickListener { PlaybackBridge.playPause(root.context); flash() }; next.setOnClickListener { PlaybackBridge.next(root.context); flash() }
        val lp = FrameLayout.LayoutParams(-1, FrameLayout.LayoutParams.WRAP_CONTENT, Gravity.BOTTOM); lp.setMargins(32, 0, 32, 32); root.addView(panel, lp); panel.visibility = View.GONE
    }

    private fun button(icon: Int, description: String) = ImageButton(root.context).apply { setImageResource(icon); contentDescription = description; setColorFilter(Color.WHITE); background = null; isFocusable = true; isFocusableInTouchMode = true; setPadding(14, 14, 14, 14) }
    ...
    fun updateProgress(positionMs: Long, durationMs: Long) { if (durationMs <= 0L) return; progress.progress = (positionMs.toDouble() / durationMs * 1000).toInt().coerceIn(0, 1000); elapsed.text = formatTime(positionMs); duration.text = formatTime(durationMs) }
