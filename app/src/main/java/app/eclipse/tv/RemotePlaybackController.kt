    ...
    fun handle(context: Context, webView: WebView, keyCode: Int): Boolean {
        when (keyCode) {
            ...
            KeyEvent.KEYCODE_MEDIA_NEXT -> {
                webView.post {
                    webView.evaluateJavascript(
                        "document.querySelector('[aria-label*=\"Next\" i],[aria-label*=\"Nächster\" i],[data-testid*=\"next\" i]')?.click();",
                        null
                    )
                }
                return true
            ...
            KeyEvent.KEYCODE_MEDIA_PREVIOUS -> {
                webView.post {
                    webView.evaluateJavascript(
                        "document.querySelector('[aria-label*=\"Previous\" i],[aria-label*=\"Zurück\" i],[data-testid*=\"previous\" i]')?.click();",
                        null
                    )
                }
                return true
